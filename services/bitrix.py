...
class BitrixAPI:
    ...
    async def get_deal_touches(self, deal_id: str) -> Optional[Dict]:
        """
        Получает информацию о касаниях с клиентом по ID сделки.
        """
        response = await self._call_method('crm.timeline.comment.list', {
    ...
    async def get_contact_info(self, contact_id: int) -> Optional[Dict]:
        ...
        """
        response = await self._call_method(
            'crm.contact.get', {'id': contact_id}
            )
        if response is None:
            logger.error(
                f"Failed to retrieve contact data for ID: {contact_id}"
                )
        return response

    async def get_deal_client_touches(self, deal_id: str) -> Optional[Dict]:
        """
        Получает информацию о касаниях
        склиентом из кастомной сущности (ID: 190)
        """
        try:
            ...
            if response and response.get('result'):
                logger.info(
                    f"Found {len(
                        ...
                        )} client touches for deal {deal_id}")
            else:
